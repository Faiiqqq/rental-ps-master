            ...
            <form action="{{ route('log.index') }}" method="GET" class="flex flex-col md:flex-row gap-4 justify-between">
                ...
                <div class="flex gap-2">
                    <input type="text" name="search" value="{{ request('search') }}"
                        ...
                        class="text-sm border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 p-2 w-full md:w-64">
                    ...
                    @if(request('search') || request('role'))
                        <a href="{{ route('log.index') }}" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-lg text-sm hover:bg-gray-300">
                            Reset
                        </a>
                    @endif
                </div>
                        ...
                        <tr class="hover:bg-gray-50 transition duration-150">
                            ...
                            <td class="px-6 py-4">
                                <span class="font-semibold 
                                    {{ $log->aksi == 'Login' ? 'text-green-600' : 
                                      ($log->aksi == 'Logout' ? 'text-red-500' : 'text-gray-700') }}">
                                    {{ $log->aksi }}
                                </span>
                            </td>
